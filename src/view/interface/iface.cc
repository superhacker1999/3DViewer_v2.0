...
s21::iface::iface(QWidget* parent) : QWidget(parent), ui(new s21::Ui::iface) {
  ui->setupUi(this);
  shift_buttons_ = {ui->x_add, ui->x_sub, ui->y_add,
                    ui->y_sub, ui->z_add, ui->z_sub};
  spin_buttons_ = {ui->x_rot_add, ui->x_rot_sub, ui->y_rot_add,
                   ui->y_rot_sub, ui->z_rot_add, ui->z_rot_sub};
  ConnectButtons_();
}

s21::iface::~iface() { delete ui; }
...
void s21::iface::SpinScene_() {
  ...
  if (button == ui->x_rot_add) {
    ui->x_rot_value->setText(
        QString::number(ui->x_rot_value->text().toInt() + 1));
    emit onOXAdd();
    ...
    ui->y_rot_value->setText(
        QString::number(ui->y_rot_value->text().toInt() + 1));
    emit onOYAdd();
    ...
    ui->z_rot_value->setText(
        QString::number(ui->z_rot_value->text().toInt() + 1));
    emit onOZAdd();
  } else {
    ui->z_rot_value->setText(
        QString::number(ui->z_rot_value->text().toInt() - 1));
...
void s21::iface::ZoomChange_() {
  ...
  if (button == ui->zoom_add) {
    ui->zoom_value->setText(
        QString::number(ui->zoom_value->text().toInt() + 10));
    emit onZoomIn();
  } else {
    ui->zoom_value->setText(
        QString::number(ui->zoom_value->text().toInt() - 10));
...
void s21::iface::ConnectButtons_() {
  ...
  connect(ui->line_type, SIGNAL(currentTextChanged(QString)), this,
          SLOT(ChangeLineType_()));
  connect(ui->line_thickness, SIGNAL(valueChanged(int)), this,
          SLOT(ChangeLineWidth_()));
  connect(ui->tops_type, SIGNAL(currentTextChanged(QString)), this,
          SLOT(ChangeDotsType_()));
  connect(ui->tops_size, SIGNAL(valueChanged(int)), this,
          SLOT(ChangeDotsSize_()));
...
void s21::iface::ChangeLineType_() { emit onLineSettingsChanged(); }

void s21::iface::ChangeLineWidth_() { emit onLineSettingsChanged(); }

void s21::iface::ChangeDotsType_() { emit onDotsSettingsChanged(); }

void s21::iface::ChangeDotsSize_() { emit onDotsSettingsChanged(); }
