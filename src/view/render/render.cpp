...
void s21::render::SetData(s21::scene_data* data, double* min, double* max,
                          ...
                          bool* line_striple, int* line_width,
                          int* display_dots, float* dots_size) {
  data_ = data;
  dot_min_ = min;
  dot_max_ = max;
...
void s21::render::ChangeDots_() {
  ...
  if (*display_dots_ > 0) {
    ...
    glDrawElements(GL_POINTS, data_->second.size(), GL_UNSIGNED_INT,
                   data_->second.data());
  }
  delete[] colors;
}
...
void s21::render::initializeGL() {
  if (is_settings_loaded_) {
    ...
    glLoadIdentity();
    glOrtho(-*(dot_max_), *dot_max_, -*(dot_max_), *dot_max_, *dot_max_,
            -*(dot_max_));
...
void s21::render::resizeGL(int w, int h) {
  if (is_settings_loaded_) glViewport(0, 0, w, h);
}
...
void s21::render::paintGL() {
  if (is_settings_loaded_) {
    ...
    ChooseLinesType_();
    glLineWidth(*line_width_);
    glDrawElements(GL_LINES, data_->second.size(), GL_UNSIGNED_INT,
                   data_->second.data());
